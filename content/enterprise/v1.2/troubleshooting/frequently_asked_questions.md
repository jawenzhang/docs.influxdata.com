...
In InfluxEnterprise versions 1.2.0-1.2.1, the system sets the [`max-row-limit` configuration option](/enterprise/v1.2/administration/configuration/#max-row-limit-0) to 10,000 by default.
...
This issue is fixed in versions 1.2.2+.
In versions 1.2.2+, `max-row-limit` is set to `0` by default.
...
This issue is fixed in versions 1.2.3+.
